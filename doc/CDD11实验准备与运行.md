...
脚本按顺序下载、核验官方 SHA-256、安全解压、检查文件名与尺寸、计算清晰内容摘要、生成清单。`preparation_status.json` 记录阶段和异常；只有 `phase=complete` 才表示全流程完成。下载采用 4 路 HTTP Range、64 MiB 分段，每段可续传；已完成分段不重复下载。整包 SHA-256 通过后清理分段，保留官方压缩包。清单不覆盖既有划分。
...
- 用户明确要求第 4 条暂不改动。CPU 预检已记录曲线偏置中的非有限值，但当前不据此修改公式、曲线代码或初始化，待该条讨论时结合引用来源与实现核对。
...
- 服务器 14 项基础测试通过；四种模型前后向检查、完整损失 CPU 计算，以及 CIDNet 两步训练、恢复、测试集成检查已完成。
- 启用曲线的配置存在 `i_curve.curve_predictor.4.bias` 非有限参数；本次前向和梯度检查为有限值。此为待核查记录，不能推导旧实验失效。
- 报告：服务器 `results/revision_preflight_20260910.json`，本地 `doc/CDD11流程预检报告_20260910.json`；报告中的合成样本指标不用于论文。
- 数据准备正在服务器后台运行。查看 `datasets/CDD-11/preparation_status.json`，只有 `phase=complete` 才表示真实数据已校验并生成完整清单；`phase=failed` 时查看同目录 `preparation.log`。
- 本阶段未启动正式 GPU 长训练。

## Windows 实时进度页面

在本地 `D:\HVI-CIDNet-master` 执行 `python -B -m revision.dashboard`，浏览器打开 <http://127.0.0.1:8766>。

页面每 10 秒通过 SSH 只读采样，显示两个压缩包的下载百分比、近期接收速度、下载预计剩余时间和处理阶段。HTTP 分段按实际接收字节统计；仅旧 Xet 稀疏文件使用磁盘块估算，不把逻辑文件大小当作下载完成。关闭页面不影响服务器后台下载。

用户最新安排：2026-09-10 当天不启动正式训练，留到次日；届时先核实 DA-Mamba 已结束、GPU 空闲和数据准备状态。
